...
export default function FluidGlass({ mode = 'lens', lensProps = {}, barProps = {}, cubeProps = {}, children }) {
    ...
    return (
        <Canvas camera={{ position: [0, 0, 20], fov: 15 }} gl={{ alpha: true }} style={{ width: '100%', height: '100%' }}>
            <ScrollControls damping={0.2} pages={1.2} distance={0.4}>
                {mode === 'bar' && <NavItems items={navItems} />}
                <Wrapper modeProps={modeProps}>
                    <Scroll>
                        {/* <Typography /> */} {/* Removing text to avoid clash */}
                        {/* <Images /> */} {/* Removing images to avoid clash */}
                    </Scroll>
...
const ModeWrapper = memo(function ModeWrapper({
    ...
    useFrame((state, delta) => {
        ...
        if (modeProps.scale == null) {
            ...
            const desired = maxWorld / 2; // Approximate width for standard primitives
    ...
    return (
        <>
            ...
            <mesh ref={ref} scale={scale ?? 0.15} rotation-x={Math.PI / 2} {...props}>
                {shape === 'lens' && <cylinderGeometry args={[1, 1, 0.4, 32]} rotation={[Math.PI / 2, 0, 0]} />}
                {(shape === 'cube' || shape === 'bar') && <boxGeometry args={[1, 1, 1]} />}
                <MeshTransmissionMaterial
...
function Lens({ modeProps, ...p }) {
    return <ModeWrapper shape="lens" followPointer modeProps={modeProps} {...p} />;
}

function Cube({ modeProps, ...p }) {
    return <ModeWrapper shape="cube" followPointer modeProps={modeProps} {...p} />;
}
